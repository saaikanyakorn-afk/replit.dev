...
| N8 | **SMTP Email Config (Platform)** — พี่ทรายต้องการตั้งค่า SMTP สำหรับส่งอีเมลจากระบบ (WHT cert email etc.). สร้างหน้า `/platform/email-config` ใหม่ + เพิ่ม "ตั้งค่า Email" ในเมนูซ้าย Platform. **⚠️ BUG พบตอน test:** API `/api/sysadmin/smtp-config` ต้องการ SysAdmin session แต่พี่ทรายเป็น Platform user (super_admin role) → ได้รับ error "กรุณาเข้าสู่ระบบ SysAdmin". **ต้องแก้:** เปลี่ยน endpoint ใน `email-config.tsx` ให้ใช้ `/api/settings/smtp` (routes ใน `doc-settings-routes.ts` ที่ requireAdmin) แทน `/api/sysadmin/smtp-config`. Files changed so far: `client/src/pages/platform/email-config.tsx` (NEW), `client/src/components/platform-layout.tsx` (เพิ่ม Mail nav), `client/src/App.tsx` (เพิ่ม route). **Status: ❌ NOT WORKING — API mismatch** | ❌ BUG - needs fix | Kai implements |
