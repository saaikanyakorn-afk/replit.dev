...
export default function StockCardPage(props: { Wrapper?: React.ComponentType<{ children: React.ReactNode }> } & Record<string, any>) {
  ...
  function handleRefClick(refType: string | null, refId: number | null, docNo?: string | null) {
    if (!refType || !refId) return;
    const info = REF_TYPE_LABELS[refType];
    if (!info || !info.path) return;
    const search = docNo ? `?search=${encodeURIComponent(docNo)}` : '';
                                ...
                                {refLabel ? (
                                  canClick ? (
                                    <button
                                      ...
                                      onClick={() => handleRefClick(m.referenceType, m.referenceId, (m as any).docNo)}
