...
export default function AccountStatementPage() {
  ...
  const { data: statement, isLoading } = useQuery<any[]>({
    queryKey: ["/api/reports/account-statement", companyId, startDate, endDate, accountCode],
    queryFn: async () => {
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2" style={{ color: "#03c9d7" }}>
            <FileText className="h-5 w-5" /> A7: Statement ตามรหัสบัญชี
          </h1>
          <p className="text-sm text-gray-500">แสดงรายการเคลื่อนไหวของบัญชีแต่ละรหัส พร้อมยอดยกมาและยอดสะสม</p>
        </div>
      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-gray-50">
                      ...
                      <th className="text-left px-2 py-1.5 w-28">อ้างอิง</th>
                  ...
                  <tbody>
                    <tr className="border-b bg-blue-50/50">
                      <td colSpan={5} className="px-2 py-1.5 text-xs font-medium text-gray-600">ยอดยกมา</td>
                    ...
                    <tr className="border-t-2 bg-gray-50 font-semibold">
                      <td colSpan={3} className="px-2 py-2 text-xs">รวม</td>
