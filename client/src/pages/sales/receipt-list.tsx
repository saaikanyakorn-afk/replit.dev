...
import {
  ...
  AlertCircle, XCircle, Copy, MoreHorizontal, FileText, Link2, MessageSquare, Printer, MailCheck,
  BookOpen, ExternalLink, Calendar as CalendarIcon, Paperclip, FileDown, Upload
} from "lucide-react";
          ...
          <CardHeader className="p-3 border-b space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button data-testid="button-import" variant="outline" onClick={() => navigate("/sales/receipt/import")} className="h-9 text-sm px-4 border-[#05b187] text-[#05b187]">
                          ...
                          <TableCell>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-56 text-sm">
                                ...
                                <DropdownMenuItem onClick={() => setEmailDialog({ open: true, id: rc.id, email: "", docNo: rc.receiptNo, customerName: rc.customerName || "" })} className="flex gap-2" style={{ color: "var(--theme-primary)" }}>
      ...
      {emailDialog && (
        <SendEmailDialog
          ...
          onConfirm={async (email) => {
            const res = await apiRequest("POST", `/api/documents/receipt/${emailDialog.id}/send-email`, { recipientEmail: email, recipientName: emailDialog.customerName });
            const data = await res.json();
