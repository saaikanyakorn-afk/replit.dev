                        ...
                        <TableRow data-testid={`row-tax-invoice-${inv.id}`} className={`hover:bg-slate-50/50 border-b ${selectedIds.has(inv.id) ? "bg-purple-50/50" : ""}`}>
                          ...
                          <TableCell className="pt-2.5 pb-2.5">
                            ...
                            <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-1 text-xs text-muted-foreground">
                              {(() => {
                                const tivPaid = inv.paymentMethod && inv.paymentMethod !== "เครดิต";
                                if (!inv.paymentMethod && !tivPaid) return null;
                                if (tivPaid) {
                          ...
                          <TableCell className="text-center pt-2.5">
                            {(() => {
                              ...
                              const tivIsPaid = inv.paymentMethod && inv.paymentMethod !== "เครดิต";
                              const effectiveStatus = isCashMethod(inv.paymentMethod)
                                ? "cash"
                                : tivIsPaid
                                  ? (["paid", "transfer_done", "completed", "cash", "credit_card", "cheque_done"].includes(inv.status) ? inv.status : "paid")
