...
import {
  ...
  RotateCcw, Paperclip, FileOutput, Receipt, ShoppingCart, Link2, Copy, AlertCircle, X, FileDown,
  CheckCircle2, XCircle, Bell, Search, Loader2, Clock
} from "lucide-react";
...
export default function QuotationForm() {
  ...
  const [matchNew] = useRoute("/sales/quote/new");
  const [matchEdit, paramsEdit] = useRoute("/sales/quote/edit/:id");
  ...
  const [form, setForm] = useState({
    quotationNo: "",
    quotationDate: toLocalDateStr(new Date()),
    validUntil: "",
    ...
    currencyCode: "THB",
    exchangeRate: "1",
    customerResponse: "" as string,
    customerResponseNote: "" as string,
  ...
  const { prefixOptions, defaultPrefix } = usePrefixOptions("quotation", docSettings);
  useEffect(() => {
  ...
  const cloneId = isNew ? new URLSearchParams(searchString).get("clone") : null;
  ...
  useEffect(() => {
    ...
    if (isNew && cloneId) {
      (async () => {
        try {
          const res = await fetch(`/api/quotations/${cloneId}`, { credentials: "include" });
          if (res.ok) {
            ...
            setForm({
              quotationNo: "",
              quotationDate: toLocalDateStr(new Date()),
              validUntil: toLocalDateStr(new Date()),
      ...
      const pipelineCustomerName = params.get("customerName");
      const pipelineContactPhone = params.get("contactPhone");
      const pipelineContactEmail = params.get("contactEmail");
      const pipelineSalesperson = params.get("salesperson");
      const pipelineNotes = params.get("notes");
      if (pipelineCustomerName || pipelineSalesperson || pipelineNotes) {
        setForm(prev => ({
          ...prev,
          quotationNo: "",
          validUntil: toLocalDateStr(new Date()),
          customerName: pipelineCustomerName || prev.customerName,
          contactPhone: pipelineContactPhone || prev.contactPhone,
          contactEmail: pipelineContactEmail || prev.contactEmail,
          salesperson: pipelineSalesperson || prev.salesperson,
          notes: pipelineNotes || prev.notes,
        }));
      } else {
        setForm(prev => ({
          ...prev,
      ...
      (async () => {
        try {
          const res = await fetch(`/api/quotations/${editingId}`, { credentials: "include" });
          if (res.ok) {
            const data = await res.json();
            setPriceMode(data.priceMode || "excluded");
            setForm({
              ...
              customerResponseNote: data.customerResponseNote || "",
              sellerBranchId: data.sellerBranchId || selectedCompany?.sellerBranchId || "00000",
            });
            if (data.discountType === "percent") setDiscountMode("percent");
            if (data.items && data.items.length > 0) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "สร้างใบเสนอราคาสำเร็จ", variant: "success" as any });
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      toast({ title: newStatus === "approved" ? "อนุมัติใบเสนอราคาสำเร็จ" : newStatus === "pending_approval" ? "ส่งขออนุมัติแล้ว" : newStatus === "rejected" ? "ปฏิเสธใบเสนอราคาแล้ว" : "อัพเดทสถานะสำเร็จ", variant: "success" as any });
    },
    onError: (err: any) => toast({ title: "เกิดข้อผิดพลาด", description: err.message, variant: "destructive" }),
  });

  const { data: approvalSettings = [] } = useQuery<any[]>({
    queryKey: ["/api/approval-settings", companyId],
    queryFn: async () => {
      const r = await fetch(`/api/approval-settings?companyId=${companyId}`, { credentials: "include" });
      if (!r.ok) return [];
  ...
  const approvalEnabled = approvalSettings.some((s: any) => s.documentType === "QO" && s.enabled);

  const requestApprovalMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/approval-requests", {
        companyId,
        documentType: "QO",
        documentId: editingId,
        documentNumber: form.quotationNo,
        amount: form.grandTotal,
        contactName: form.contactName || form.contactOrganization,
      });
      return res.json();
    },
    onSuccess: () => {
      ...
      toast({ title: "ส่งขออนุมัติเรียบร้อย", description: "ระบบได้แจ้งเตือนผู้อนุมัติทาง LINE แล้ว", variant: "success" as any });
    },
    onError: (err: any) => toast({ title: "เกิดข้อผิดพลาด", description: err.message, variant: "destructive" }),
  });
  ...
  function handleCustomerSelect(contactId: string) {
    ...
    if (c) {
      setForm(prev => {
        ...
        if (prev.quotationDate) {
          const base = new Date(prev.quotationDate + "T00:00:00");
          base.setDate(base.getDate() + days);
          newValidUntil = toLocalDateStr(base);
        }
        return {
          ...prev,
          customerId: c.id,
  ...
  function calcTotals() {
    ...
    const discAmount = discIsPercent
      ? rawTotal * discPercVal / 100
      : (parseFloat(discRaw) || 0);
  ...
  const isLocked = !isNew && ["pending_approval", "rejected", "converted"].includes(form.status);
    ...
    <Layout>
      <div className="space-y-0">
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
          ...
          <span className="cursor-pointer hover:text-[var(--theme-primary)]" onClick={() => navigate("/sales/quote")}>ใบเสนอราคา (QO)</span>
          <span>/</span>
          <span className="text-foreground font-medium">{editingId ? (isLocked ? "ดูใบเสนอราคา" : "แก้ไขใบเสนอราคา") : "สร้างใบเสนอราคา"}</span>
        </div>

        <div className="flex items-center gap-3 mb-4">
          <Button data-testid="button-back" variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate("/sales/quote")}>
          ...
          <h1 className="text-xl font-heading font-medium" data-testid="text-page-title">
            {editingId ? (isLocked ? "ดูใบเสนอราคา" : "แก้ไขใบเสนอราคา") : "สร้างใบเสนอราคา"}
          </h1>
        </div>

        {editingId && form.customerResponse === "request_edit" && form.status !== "approved" && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 mb-4 flex items-center gap-2 text-sm text-blue-800">
            ...
            <span>ลูกค้าขอแก้ไขใบเสนอราคานี้{form.customerResponseNote ? `: "${form.customerResponseNote}"` : ""}</span>
          </div>
        )}

        {editingId && form.customerResponse === "confirmed" && form.status !== "approved" && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-3 mb-4 flex items-center gap-2 text-sm text-emerald-800">
            <CheckCircle2 className="h-4 w-4 flex-shrink-0 text-emerald-600" />
            <span>ลูกค้ายืนยันใบเสนอราคานี้แล้ว กรุณากดอนุมัติเพื่อดำเนินการต่อ</span>
          </div>
        )}
        ...
        {isLocked && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mb-4 flex items-center gap-2 text-sm text-amber-800">
            ...
            <span>เอกสารนี้มีสถานะ <strong>{form.status === "pending_approval" ? "รออนุมัติ" : form.status === "rejected" ? "ปฏิเสธแล้ว" : "แปลงแล้ว"}</strong> ไม่สามารถแก้ไขได้ หากต้องการแก้ไขให้ใช้ "คัดลอกเอกสาร" เพื่อสร้างใหม่</span>
          </div>
        )}
        ...
        <div className={`bg-white border rounded-lg shadow-sm ${isLocked ? "opacity-80" : ""}`}>
          <fieldset disabled={isLocked} className="p-2 sm:p-4 space-y-4">
            <div className="border rounded overflow-hidden doc-header-table">
              <table className="w-full text-sm table-fixed">
                ...
                <tbody>
                  ...
                  {/* Row 2: ชื่อคู่ค้า(4) | สาขา(2) | วันที่ออกใบเสนอราคา(2) */}
                  <tr className="border-b border-slate-300">
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      <div className="text-[10px] text-slate-400 mb-0.5">วันที่ออกใบเสนอราคา</div>
                      <DatePicker data-testid="input-quotation-date" value={form.quotationDate} onChange={v => { const days = parseInt(form.creditDays) || 0; let newValidUntil = v || form.validUntil; if (v) { const base = new Date(v + "T00:00:00"); base.setDate(base.getDate() + days); newValidUntil = toLocalDateStr(base); } setForm(p => ({ ...p, quotationDate: v, validUntil: newValidUntil })); }} dateFormat={dateFmt} dateEra={dateEra} className="w-full" />
                    </td>
                  </tr>

                  {/* Row 3: ที่อยู่(4) | เลขผู้เสียภาษี+เครดิต stacked(2) | วันที่+กำหนดส่งของ stacked(2) */}
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={4}>
                    ...
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <div className="mt-1.5">
                        ...
                        <Input data-testid="input-credit-days" inputMode="numeric" value={form.creditDays || ""} onChange={e => { const v = e.target.value; if (/^\d*$/.test(v)) { const days = parseInt(v) || 0; let newValidUntil = form.validUntil; if (form.quotationDate) { const base = new Date(form.quotationDate + "T00:00:00"); base.setDate(base.getDate() + days); newValidUntil = toLocalDateStr(base); } setForm(p => ({ ...p, creditDays: v, validUntil: newValidUntil })); } }} className="h-7 text-xs border-dashed" placeholder="0" />
                      </div>
                    </td>
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      <div className="text-[10px] text-slate-400 mb-0.5">กำหนดส่งของ</div>
                      <DatePicker data-testid="input-valid-until" value={form.validUntil} onChange={v => setForm(p => ({ ...p, validUntil: v }))} dateFormat={dateFmt} dateEra={dateEra} className="w-full" />
                    </td>
                  </tr>

                  {/* Row 4: อีเมล์(2) | โทรศัพท์(2) | เงื่อนไข(2) | เลขที่ใบเสนอราคา(2) */}
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                    ...
                    <td className="px-3 pt-1.5 pb-1 border-r align-top bg-amber-50/70" colSpan={2}>
                      <div className="text-[10px] text-amber-600 font-semibold mb-0.5">เงื่อนไขการชำระเงิน</div>
                      <Input data-testid="input-payment-terms" value={form.paymentTerms || ""} onChange={e => setForm(p => ({ ...p, paymentTerms: e.target.value }))} className="h-7 text-xs border-dashed border-amber-300 bg-white focus:border-amber-500 focus:ring-amber-200" placeholder="เช่น มัดจำ 50%" />
                    </td>
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      <div className="text-[10px] text-slate-400 mb-0.5">เลขที่ใบเสนอราคา</div>
                      <div className="flex items-center gap-1">
                        ...
                        <Input data-testid="input-quotation-no" value={form.quotationNo} onChange={e => setForm(p => ({ ...p, quotationNo: e.target.value }))} className="h-7 text-xs border-dashed flex-1" placeholder="AUTO" />
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <Input data-testid="input-salesperson" list="salesperson-list" value={form.salesperson} onChange={e => setForm(p => ({ ...p, salesperson: e.target.value }))} className="h-7 text-xs border-dashed" placeholder="เลือกหรือพิมพ์ชื่อ" />
                    ...
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <div className="flex items-center gap-1">
                        <Input data-testid="input-department" list="department-list" value={form.department} onChange={e => setForm(p => ({ ...p, department: e.target.value }))} className="h-7 text-xs border-dashed flex-1" placeholder="เลือกหรือพิมพ์แผนก" />
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {form.currencyCode !== "THB" && (
                          <>
                            ...
                            <FetchRateButton currency={form.currencyCode} date={form.quotationDate} onRate={r => setForm(p => ({ ...p, exchangeRate: String(r) }))} rateType="buying_transfer" />
                          </>
                        )}
                      </div>
                    </td>
              ...
              <div className="w-full sm:flex-1 space-y-3">
                ...
                <div className="flex items-center gap-2 w-full">
                  <MultiFileAttachment
                    ...
                    testIdPrefix="quotation-attachment"
                  />
            ...
            <div className="flex items-center justify-center gap-3 pt-4 border-t flex-wrap">
              ...
              {editingId && form.status === "draft" && approvalEnabled && (
                <Button
                  data-testid="button-request-approval"
                  onClick={() => requestApprovalMutation.mutate()}
                  disabled={requestApprovalMutation.isPending || isSaving}
                  size="sm"
                  variant="outline"
                  className="h-9 px-6 gap-1.5 text-sm border-amber-400 text-amber-700 hover:bg-amber-50"
                >
                  <Clock className="h-3.5 w-3.5" />
                  {requestApprovalMutation.isPending ? "กำลังส่ง..." : "ขออนุมัติ"}
                </Button>
              )}
              {editingId && form.status === "draft" && !approvalEnabled && (
                <Button
                  data-testid="button-approve-direct"
              ...
              {editingId && (form.status === "draft" || form.status === "pending_approval") && (
                <Button
                  data-testid="button-cancel-doc"
                  onClick={() => statusMutation.mutate({ id: editingId, status: "rejected" })}
                  disabled={statusMutation.isPending || isSaving}
                  variant="outline"
                  size="sm"
            ...
            {isSaved && (
              <div className="border-t pt-4">
                <div className="text-xs font-semibold text-slate-600 mb-2 flex items-center gap-1.5">
                  ...
                  เชื่อมโยงเอกสาร — ออกเอกสารต่อจากใบเสนอราคานี้
                </div>
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="h-8 gap-1.5 border-[var(--theme-primary)]/30 bg-[var(--theme-primary-light)] hover:bg-[var(--theme-primary-light)] text-[var(--theme-primary)]"
                    onClick={() => navigate(`/sales/order/new?fromQuote=${savedId || editingId}`)}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => navigate(`/sales/invoice/new?fromQuotation=${savedId || editingId}`)}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => navigate(`/sales/tax-invoice/new?fromQuotation=${savedId || editingId}`)}
                  >
      ...
      <RelatedDocuments docType="quotation" docId={savedId || editingId} />
    </Layout>
  );
}
