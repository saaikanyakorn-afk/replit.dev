...
export default function DepositForm() {
  ...
  const [matchNew] = useRoute("/sales/deposit/new");
  const [matchEdit, paramsEdit] = useRoute("/sales/deposit/edit/:id");
  ...
  const { data: contacts = [] } = useQuery<Contact[]>({
    ...
    queryFn: async () => {
      if (!companyId) return [];
      const res = await fetch(`/api/contacts?companyId=${companyId}&type=customer`, { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
    enabled: !!companyId,
  ...
  const { prefixOptions, defaultPrefix } = usePrefixOptions("deposit", docSettings);
  useEffect(() => {
  ...
  useEffect(() => {
    if (isNew && activePaymentMethods.length > 0 && (form.paymentMethod === "" || form.paymentMethod === "transfer")) {
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      (async () => {
        try {
          const res = await fetch(`/api/deposit-receipts/${editingId}`, { credentials: "include" });
          if (res.ok) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      const res = await apiRequest("POST", "/api/deposit-receipts", data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/deposit-receipts"] });
      toast({ title: "สร้างใบรับเงินมัดจำสำเร็จ", variant: "success" as any });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const res = await apiRequest("PATCH", `/api/deposit-receipts/${id}`, data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/deposit-receipts"] });
    ...
    <Layout>
      <div className="space-y-0">
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
          ...
          <span className="cursor-pointer hover:text-[var(--theme-primary)]" onClick={() => navigate("/sales/deposit")}>ใบรับเงินมัดจำ (DP)</span>
          <span>/</span>
          <span className="text-foreground font-medium">{editingId ? (isLocked ? "ดูใบรับเงินมัดจำ" : "แก้ไขใบรับเงินมัดจำ") : "สร้างใบรับเงินมัดจำ"}</span>
        </div>

        <div className="flex items-center gap-3 mb-4">
          <Button data-testid="button-back" variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate("/sales/deposit")}>
          ...
          <h1 className="text-xl font-heading font-medium" data-testid="text-page-title">
            {editingId ? (isLocked ? "ดูใบรับเงินมัดจำ" : "แก้ไขใบรับเงินมัดจำ") : "สร้างใบรับเงินมัดจำ"}
          </h1>
        </div>
            ...
            <div className="border rounded overflow-hidden doc-header-table">
              <table className="w-full text-sm table-fixed">
                ...
                <tbody>
                            ...
                            <div className="absolute z-50 top-full left-0 mt-1 bg-white border rounded-md shadow-lg max-h-48 overflow-y-auto min-w-[220px]">
                              {filtered.map(c => (
                                <button key={c.id} className="w-full text-left px-3 py-2 text-xs hover:bg-sky-50 border-b last:border-b-0"
                                  onMouseDown={e => { e.preventDefault(); handleCustomerSelect(String(c.id)); setCustomerCodeSearch(null); setCustomerSearch(null); setShowCustomerCodeDropdown(false); }}>
                  ...
                  {/* Row 2: ชื่อลูกค้า(4) | สาขา(2) | เลขที่ใบกำกับภาษี(2, bg-blue) */}
                  <tr className="border-b border-slate-300">
                        ...
                        {showCustomerDropdown && (() => {
                          ...
                          return (
                            <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border rounded-md shadow-lg max-h-48 overflow-y-auto">
                              {filtered.map(c => (
                                <button key={c.id} className="w-full text-left px-3 py-2 text-xs hover:bg-sky-50 border-b last:border-b-0"
                                  onMouseDown={e => { e.preventDefault(); handleCustomerSelect(String(c.id)); setCustomerSearch(null); setCustomerCodeSearch(null); setShowCustomerDropdown(false); }}>
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top bg-blue-50/40" colSpan={2}>
                      ...
                      <Input data-testid="input-tax-invoice-ref" value={(form as any).taxInvoiceRef || ""} onChange={e => setForm(p => ({ ...p, taxInvoiceRef: e.target.value } as any))} className="h-7 text-xs border-dashed border-blue-300 bg-white focus:border-blue-500 focus:ring-blue-200" placeholder="เลขที่ใบกำกับภาษี" />
                    </td>
                  </tr>

                  {/* Row 3: ที่อยู่(4) | เลขผู้เสียภาษี+เครดิต(2) | วันที่ออก(2) */}
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={4}>
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      ...
                      <div className="flex items-center gap-1">
                        <Select value={form.docPrefix} onValueChange={v => setForm(p => ({ ...p, docPrefix: v }))}>
                          <SelectTrigger data-testid="select-doc-prefix" className="h-7 text-xs w-[50px] border-dashed px-1.5">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                  ...
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <Input data-testid="input-salesperson" list="emp-list-dp" value={form.salesperson} onChange={e => setForm(p => ({ ...p, salesperson: e.target.value }))} className="h-7 text-xs border-dashed" placeholder="เลือกหรือพิมพ์ชื่อ" />
                      <datalist id="emp-list-dp">
                    ...
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <div className="flex items-center gap-1">
                        <Input data-testid="input-department" list="dept-list-dp" value={form.department} onChange={e => setForm(p => ({ ...p, department: e.target.value }))} className="h-7 text-xs border-dashed flex-1" placeholder="เลือกหรือพิมพ์แผนก" />
                        <datalist id="dept-list-dp">
                    ...
                    <td className="px-3 py-1.5" colSpan={8}>
                      <div className="flex items-center gap-6">
                        <div className="flex items-center gap-2">
                          <Checkbox checked={form.saveToContacts} onCheckedChange={(v) => setForm(p => ({ ...p, saveToContacts: !!v }))} disabled={!!form.customerId} />
