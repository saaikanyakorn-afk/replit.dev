...
import {
  ...
  RotateCcw, Paperclip, Copy, AlertCircle, X, FileDown, CheckCircle2, XCircle, Link2, Receipt, Search, Loader2
} from "lucide-react";
...
export default function InvoiceForm() {
  ...
  const [matchNew] = useRoute("/sales/invoice/new");
  const [matchEdit, paramsEdit] = useRoute("/sales/invoice/edit/:id");
  ...
  const [form, setForm] = useState({
    invoiceNo: "",
    invoiceDate: toLocalDateStr(new Date()),
    dueDate: "",
  ...
  useEffect(() => {
    ...
    if (!isNaN(days) && days >= 0 && form.invoiceDate) {
      const base = new Date(form.invoiceDate + "T00:00:00");
    ...
    } else if (!form.creditDays && form.invoiceDate) {
      setForm(p => ({ ...p, dueDate: p.invoiceDate }));
    }
  }, [form.creditDays, form.invoiceDate, loaded]);
  ...
  const { prefixOptions, defaultPrefix } = usePrefixOptions("invoice", docSettings);
  useEffect(() => {
  ...
  const invoiceSearchParams = isNew ? new URLSearchParams(searchString) : null;
  const fromQuotationId = invoiceSearchParams?.get("fromQuotation") || null;
  const fromSalesOrderId = invoiceSearchParams?.get("fromSalesOrder") || null;
  const copyFromId = invoiceSearchParams?.get("copyFrom") || null;

  useEffect(() => {
    if (loaded) return;
    if (isNew && fromSalesOrderId) {
      (async () => {
        try {
      ...
      (async () => {
        try {
          const res = await fetch(`/api/invoices/${copyFromId}`, { credentials: "include" });
          if (res.ok) {
            ...
            setForm(prev => ({
              ...
              creditDays: src.creditDays ? String(src.creditDays) : "",
              notes: src.notes || "",
              ...
              docPrefix: src.docPrefix || "IV",
              currencyCode: src.currencyCode || "THB",
              exchangeRate: String(src.exchangeRate || "1"),
            }));
      ...
      (async () => {
        try {
          const res = await fetch(`/api/invoices/${editingId}`, { credentials: "include" });
          if (res.ok) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      if (data.journalResult && !data.journalResult.skipped) {
        toast({ title: "สร้างใบแจ้งหนี้สำเร็จ", description: "สร้างรายการบัญชีอัตโนมัติแล้ว", variant: "success" as any });
      } else if (data.journalResult?.skipped && data.journalResult?.reason) {
        toast({ title: "สร้างใบแจ้งหนี้สำเร็จ", description: `ไม่ได้ลงบัญชี: ${data.journalResult.reason}`, variant: "default" as any });
      } else {
        toast({ title: "สร้างใบแจ้งหนี้สำเร็จ", variant: "success" as any });
      }
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: number; status: string }) => {
      const res = await apiRequest("PATCH", `/api/invoices/${id}`, { status, journalOverrideLines: journalOverrideLines || undefined });
      return res.json();
    },
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ["/api/invoices"] });
      const newStatus = data.invoice?.status || data.status;
      setForm(p => ({ ...p, status: newStatus || "approved" }));
      if (data.journalResult && !data.journalResult.skipped) {
        toast({ title: "อนุมัติสำเร็จ", description: "สร้างรายการบัญชีอัตโนมัติแล้ว", variant: "success" as any });
      } else if (data.journalResult?.skipped && data.journalResult?.reason) {
        const statusLabels: Record<string, string> = { approved: "อนุมัติแล้ว", cancelled: "ยกเลิกแล้ว", draft: "กลับเป็นแบบร่าง" };
        toast({ title: statusLabels[newStatus] || "อัพเดทสถานะสำเร็จ", description: `ไม่ได้ลงบัญชี: ${data.journalResult.reason}`, variant: "default" as any });
      } else {
        const statusLabels: Record<string, string> = { approved: "อนุมัติแล้ว", cancelled: "ยกเลิกแล้ว", draft: "กลับเป็นแบบร่าง" };
  ...
  function handleCustomerSelect(contactId: string) {
    ...
    if (c) {
      setForm(prev => {
        ...
        if (prev.invoiceDate) {
          const base = new Date(prev.invoiceDate + "T00:00:00");
          base.setDate(base.getDate() + days);
  ...
  async function handleSubmit() {
    ...
    const payload = {
      ...
      salesOrderId: fromSalesOrderId ? Number(fromSalesOrderId) : (form as any).salesOrderId || null,
      priceMode,
      docPrefix: form.docPrefix,
  ...
  const isLocked = !isNew && ["paid", "cancelled"].includes(form.status);
    ...
    <Layout>
      <div className="space-y-0">
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
          ...
          <span className="cursor-pointer hover:text-[var(--theme-primary)]" onClick={() => navigate("/sales/invoice")}>ใบแจ้งหนี้ (IV)</span>
          <span>/</span>
          <span className="text-foreground font-medium">{editingId ? (isLocked ? "ดูใบแจ้งหนี้" : "แก้ไขใบแจ้งหนี้") : "สร้างใบแจ้งหนี้"}</span>
        </div>

        <div className="flex items-center gap-3 mb-4">
          <Button data-testid="button-back" variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate("/sales/invoice")}>
          ...
          <h1 className="text-xl font-heading font-medium" data-testid="text-page-title">
            {editingId ? (isLocked ? "ดูใบแจ้งหนี้" : "แก้ไขใบแจ้งหนี้") : "สร้างใบแจ้งหนี้"}
          </h1>
        </div>
        ...
        <div className={`bg-white border rounded-lg shadow-sm ${isLocked ? "opacity-80" : ""}`}>
          <fieldset disabled={isLocked} className="p-2 sm:p-4 space-y-4">
                ...
                <tbody>
                  <tr className="border-b border-slate-300">
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      ...
                      <Input data-testid="input-ref-doc" value={form.refDoc || ""} onChange={e => setForm(p => ({ ...p, refDoc: e.target.value }))} className="h-7 text-xs border-dashed" placeholder="QO/SO number" />
                    </td>
                  </tr>

                  {/* Row 2: ชื่อคู่ค้า(4) | สาขา(2) | วันที่ออกใบแจ้งหนี้(2) */}
                  <tr className="border-b border-slate-300">
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      <div className="text-[10px] text-slate-400 mb-0.5">วันที่ออกใบแจ้งหนี้</div>
                      <DatePicker data-testid="input-invoice-date" value={form.invoiceDate} onChange={v => setForm(p => ({ ...p, invoiceDate: v }))} dateFormat={dateFmt} dateEra={dateEra} className="w-full" />
                    </td>
                  </tr>

                  {/* Row 3: ที่อยู่(4) | เลขผู้เสียภาษี+เครดิต stacked(2) | วันที่+วันครบกำหนด stacked(2) */}
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={4}>
                  ...
                  {/* Row 4: อีเมล์(2) | โทรศัพท์(2) | สถานะชำระ(2, bg-amber) | เลขที่ใบแจ้งหนี้(2) */}
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                    ...
                    <td className="px-3 pt-1.5 pb-1 border-r align-top bg-amber-50/70" colSpan={2}>
                      <div className="text-[10px] text-amber-600 font-semibold mb-0.5">สถานะการชำระ</div>
                      <Select value={form.paymentStatus || "unpaid"} onValueChange={v => setForm(p => ({ ...p, paymentStatus: v }))}>
                        <SelectTrigger data-testid="select-payment-status" className="h-7 text-xs border-dashed border-amber-300 bg-white focus:border-amber-500 focus:ring-amber-200">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      <div className="text-[10px] text-slate-400 mb-0.5">เลขที่ใบแจ้งหนี้</div>
                      <div className="flex items-center gap-1">
                        ...
                        <Input data-testid="input-invoice-no" value={form.invoiceNo} onChange={e => setForm(p => ({ ...p, invoiceNo: e.target.value }))} className="h-7 text-xs border-dashed flex-1" placeholder="AUTO" />
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <Input data-testid="input-salesperson" list="emp-list-iv" value={form.salesperson} onChange={e => setForm(p => ({ ...p, salesperson: e.target.value }))} className="h-7 text-xs border-dashed" placeholder="เลือกหรือพิมพ์ชื่อ" />
                      <datalist id="emp-list-iv">
                    ...
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <div className="flex items-center gap-1">
                        <Input data-testid="input-department" list="dept-list-iv" value={form.department} onChange={e => setForm(p => ({ ...p, department: e.target.value }))} className="h-7 text-xs border-dashed flex-1" placeholder="เลือกหรือพิมพ์แผนก" />
                        <datalist id="dept-list-iv">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {form.currencyCode !== "THB" && (
                          <>
                            ...
                            <FetchRateButton currency={form.currencyCode} date={form.invoiceDate} onRate={r => setForm(p => ({ ...p, exchangeRate: String(r) }))} rateType="buying_transfer" />
                          </>
                        )}
                      </div>
                    </td>
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="flex items-center gap-2 w-full">
                  <MultiFileAttachment
                    ...
                    testIdPrefix="invoice-attachment"
                  />
            ...
            <div className="flex items-center justify-center gap-3 pt-4 border-t">
              ...
              {editingId && !["approved", "paid", "cancelled"].includes(form.status) && (
                <Button
              ...
              <div className="border-t pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => navigate(`/sales/tax-invoice/new?fromInvoice=${savedId || editingId}`)}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => navigate(`/sales/receipt/new?fromInvoice=${savedId || editingId}`)}
                  >
      ...
      <RelatedDocuments docType="invoice" docId={savedId || editingId} />
    </Layout>
  );
}
