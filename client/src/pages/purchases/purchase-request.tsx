...
import {
  ...
  RotateCcw, Copy, AlertCircle, CheckCircle2, XCircle, Search, Loader2, Clock
} from "lucide-react";
...
export default function PurchaseRequest() {
  ...
  const [matchNew] = useRoute("/purchases/pr/new");
  const [matchEdit, paramsEdit] = useRoute("/purchases/pr/edit/:id");
  ...
  const [form, setForm] = useState({
    prNo: "",
    prDate: toLocalDateStr(new Date()),
    deliveryDate: "",
  ...
  useEffect(() => {
    ...
    if (isNew && copyFromId) {
      (async () => {
        try {
          const res = await fetch(`/api/purchase-requests/${copyFromId}`, { credentials: "include" });
          if (res.ok) {
      ...
      (async () => {
        try {
          const res = await fetch(`/api/purchase-requests/${editingId}`, { credentials: "include" });
          if (res.ok) {
            const data = await res.json();
            setPriceMode(data.priceMode || "excluded");
            setForm({
  ...
  useEffect(() => {
    ...
    if (!isNaN(days) && days >= 0 && form.prDate) {
      const base = new Date(form.prDate + "T00:00:00");
  ...
  }, [form.creditDays, form.prDate, loaded]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "สร้างใบขอซื้อสำเร็จ", variant: "success" as any });
      navigate("/purchases/pr");
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/purchase-requests"] });
      toast({ title: "อัพเดทใบขอซื้อสำเร็จ", variant: "success" as any });
    },
    onError: (err: any) => toast({ title: "เกิดข้อผิดพลาด", description: err.message, variant: "destructive" }),
  });
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      const statusLabels: Record<string, string> = { approved: "อนุมัติแล้ว", pending_approval: "ส่งขออนุมัติแล้ว", cancelled: "ยกเลิกแล้ว" };
  ...
  const isLocked = !isNew && ["pending_approval", "cancelled"].includes(form.status);
    ...
    <Layout>
      <div className="space-y-0">
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
          ...
          <span className="cursor-pointer hover:text-[var(--theme-primary)]" onClick={() => navigate("/purchases/pr")}>ใบขอซื้อ (PR)</span>
          <span>/</span>
          <span className="text-foreground font-medium">{editingId ? (isLocked ? "ดูใบขอซื้อ" : "แก้ไขใบขอซื้อ") : "สร้างใบขอซื้อ"}</span>
        </div>

        <div className="flex items-center gap-3 mb-4">
          <Button data-testid="button-back" variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate("/purchases/pr")}>
          ...
          <h1 className="text-xl font-heading font-medium" data-testid="text-page-title">
            {editingId ? (isLocked ? "ดูใบขอซื้อ" : "แก้ไขใบขอซื้อ") : "สร้างใบขอซื้อ"}
          </h1>
        </div>
        ...
        {isLocked && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mb-4 flex items-center gap-2 text-sm text-amber-800">
            ...
            <span>เอกสารนี้มีสถานะ <strong>{form.status === "pending_approval" ? "รออนุมัติ" : "ยกเลิก"}</strong> ไม่สามารถแก้ไขได้</span>
          </div>
        )}
              ...
              <table className="w-full text-sm table-fixed">
                ...
                <tbody>
                  ...
                  <tr className="border-b border-slate-300">
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      <div className="text-[10px] text-slate-400 mb-0.5">วันที่ออกใบขอซื้อ</div>
                      <DatePicker data-testid="input-pr-date" value={form.prDate} onChange={v => setForm(p => ({ ...p, prDate: v }))} dateFormat={dateFmt} dateEra={dateEra} className="w-full" />
                    </td>
                  </tr>
                  ...
                  <tr className="border-b border-slate-300">
                    ...
                    <td className="px-3 pt-1.5 pb-1 align-top" colSpan={2}>
                      <div className="text-[10px] text-slate-400 mb-0.5">เลขที่ใบขอซื้อ</div>
                      <div className="flex items-center gap-1">
                        ...
                        <Input data-testid="input-pr-no" value={form.prNo} onChange={e => setForm(p => ({ ...p, prNo: e.target.value }))} className="h-7 text-xs border-dashed flex-1" placeholder="AUTO" />
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr className="border-b border-slate-300">
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <Input data-testid="input-salesperson" list="emp-list-pr" value={form.salesperson} onChange={e => setForm(p => ({ ...p, salesperson: e.target.value }))} className="h-7 text-xs border-dashed" placeholder="เลือกหรือพิมพ์ชื่อ" />
                      <datalist id="emp-list-pr">
                    ...
                    <td className="px-3 pt-1.5 pb-1 border-r align-top" colSpan={2}>
                      ...
                      <div className="flex items-center gap-1">
                        <Input data-testid="input-department" list="dept-list-pr" value={form.department} onChange={e => setForm(p => ({ ...p, department: e.target.value }))} className="h-7 text-xs border-dashed flex-1" placeholder="เลือกหรือพิมพ์แผนก" />
                        <datalist id="dept-list-pr">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {form.currencyCode !== "THB" && (
                          <>
                            ...
                            <FetchRateButton currency={form.currencyCode} date={form.prDate} onRate={r => setForm(p => ({ ...p, exchangeRate: String(r) }))} rateType="buying_transfer" />
                          </>
                        )}
                      </div>
                    </td>
        ...
        <div className="flex items-center justify-center gap-3 mt-4">
          ...
          {editingId && form.status === "approved" && (
            <Button variant="outline" className="border-amber-400 text-amber-700 hover:bg-amber-50" onClick={() => statusMutation.mutate({ id: editingId, status: "pending_approval" })} data-testid="button-request-approval">
              <Clock className="h-4 w-4 mr-2" /> ขออนุมัติ
            </Button>
          )}
          ...
          {editingId && (form.status === "approved" || form.status === "pending_approval") && (
