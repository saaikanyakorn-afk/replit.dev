...
import { QrCode, User, Battery, CheckCircle2, Circle, Zap, Wrench, Shield, Package, ArrowLeft, Camera, Plus, Trash2, ChevronRight } from "lucide-react";
...
export default function MesScanStation() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-start pt-6 pb-10 px-4">
      <div className="w-full max-w-lg">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <QrCode className="text-cyan-400 w-6 h-6" />
            <span className="text-white font-bold text-xl">สถานียิง QR</span>
          </div>
          <Button variant="ghost" size="sm" className="text-slate-400 hover:text-white" onClick={() => navigate("/manufacturing/mes/work-orders")}>
          ...
          {["พนักงาน", "หน่วยผลิต", "Process", "เสร็จ"].map((s, i) => {
            ...
            return (
              <div key={s} className="flex items-center flex-1">
                <div className={`flex-1 h-1.5 rounded-full ${active ? "bg-cyan-500" : "bg-slate-700"}`} />
          ...
          <Card title="ขั้นตอนที่ 1: ยืนยันตัวตน" subtitle="ยิง QR บัตรพนักงานของคุณ">
            <div className="relative">
              ...
              <Input
                ...
                className="pl-10 bg-slate-800 border-slate-600 text-white placeholder:text-slate-500 h-12 text-base"
                disabled={loading}
        ...
        {step === "unit" && (
          <Card title="ขั้นตอนที่ 2: ยิง QR กล่องแบต" subtitle={`พนักงาน: ${employeeName}`}>
            <div className="flex items-center gap-2 mb-4 bg-slate-700 rounded-lg px-3 py-2">
              <User className="w-4 h-4 text-cyan-400" />
              <span className="text-cyan-300 text-sm font-medium">{employeeName}</span>
              <Button variant="ghost" size="sm" className="ml-auto text-slate-400 hover:text-white text-xs h-6 px-2" onClick={resetAll}>เปลี่ยน</Button>
            </div>
            <div className="relative">
              <QrCode className="absolute left-3 top-3 w-5 h-5 text-slate-400" />
              <Input
        ...
        {step === "process" && unitData && (
          <div className="space-y-4">
            <Card title="ขั้นตอนที่ 3: เลือก Process" subtitle={`${unitData.workOrder?.productName || ""} · หน่วยที่ ${unitData.unitNo}`}>
              <div className="flex items-center gap-2 mb-4 bg-slate-700 rounded-lg px-3 py-2 text-sm">
                <QrCode className="w-4 h-4 text-cyan-400" />
                <span className="text-slate-300 font-mono">{unitData.masterQr}</span>
                ...
                {PROCESSES.map(p => {
                  ...
                  return (
                    <button
                      ...
                      disabled={done}
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all ${done ? "border-emerald-700 bg-emerald-900/30 text-emerald-400 opacity-60 cursor-not-allowed" : next ? "border-cyan-500 bg-cyan-900/40 text-cyan-300 ring-1 ring-cyan-500" : "border-slate-600 bg-slate-800 text-slate-300 hover:border-slate-400"}`}
                      data-testid={`btn-process-${p.no}`}
                    >
                      {done ? <CheckCircle2 className="w-5 h-5 text-emerald-500 flex-shrink-0" /> : <Icon className={`w-5 h-5 flex-shrink-0 ${next ? "text-cyan-400" : "text-slate-500"}`} />}
                      <span className="font-medium">{p.label}</span>
                      {next && <span className="ml-auto text-xs text-cyan-400 bg-cyan-900 px-2 py-0.5 rounded">ถัดไป</span>}
            ...
            {selectedProcess && selectedProcess !== 2 && selectedProcess !== 5 && (
              <Card title={`ยืนยัน Process ${selectedProcess}`} subtitle={PROCESSES[selectedProcess - 1]?.label}>
                ...
                <Button variant="ghost" className="w-full mt-2 text-slate-400" onClick={() => setSelectedProcess(null)}>ยกเลิก</Button>
              </Card>
            )}
          </div>
        )}
        ...
        {step === "cell_scan" && (
          <Card title="P2: ยิง QR เซลล์แบต" subtitle={`หน่วยที่ ${unitData?.unitNo} — ยิงทีละก้อนจนครบ`}>
            <div className="mb-3 flex items-center gap-2 text-sm text-slate-400">
              ...
              <span>เซลล์ที่เพิ่มแล้ว: <strong className="text-purple-300">{cells.length} ก้อน</strong></span>
            </div>
            {cells.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3 max-h-32 overflow-y-auto">
                {cells.map((c, i) => (
                  <div key={i} className="flex items-center gap-1 bg-purple-900/40 text-purple-300 border border-purple-700 rounded px-2 py-0.5 text-xs font-mono">
                    {i + 1}. {c}
                    <button onClick={() => setCells(cs => cs.filter((_, j) => j !== i))} className="text-purple-500 hover:text-red-400">
            ...
            <div className="relative mb-3">
              ...
              <Input
                ...
                className="pl-10 bg-slate-800 border-purple-700 text-white placeholder:text-slate-500 h-12 text-base"
                data-testid="input-cell-qr"
              />
            </div>
            <Button className="w-full h-10 mb-3 bg-purple-600 hover:bg-purple-700" onClick={() => handleCellScan(cellInput)} disabled={!cellInput.trim()}>
              <Plus className="w-4 h-4 mr-1" /> เพิ่มเซลล์
            </Button>
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1 border-slate-600 text-slate-300 hover:bg-slate-700" onClick={() => { setStep("process"); setSelectedProcess(null); }}>ยกเลิก</Button>
        ...
        {step === "balance" && (
          <Card title="P5: Balance Cell" subtitle={`หน่วยที่ ${unitData?.unitNo}`}>
            <div className="space-y-3">
              <div>
                <Label className="text-slate-300 text-sm">ค่าแรงดันก่อน Balance (V)</Label>
                <Textarea value={balanceBefore} onChange={e => setBalanceBefore(e.target.value)} placeholder="เช่น: Cell1=3.2V, Cell2=3.18V, ..." className="bg-slate-800 border-slate-600 text-white placeholder:text-slate-500 text-sm mt-1" rows={3} data-testid="input-balance-before" />
              </div>
              <div>
                <Label className="text-slate-300 text-sm">ค่าแรงดันหลัง Balance (V)</Label>
                <Textarea value={balanceAfter} onChange={e => setBalanceAfter(e.target.value)} placeholder="เช่น: Cell1=3.28V, Cell2=3.28V, ..." className="bg-slate-800 border-slate-600 text-white placeholder:text-slate-500 text-sm mt-1" rows={3} data-testid="input-balance-after" />
              </div>
              <div>
                <Label className="text-slate-300 text-sm">หมายเหตุ</Label>
                <Textarea value={balanceNotes} onChange={e => setBalanceNotes(e.target.value)} className="bg-slate-800 border-slate-600 text-white placeholder:text-slate-500 text-sm mt-1" rows={2} data-testid="input-balance-notes" />
              </div>
              <div>
                <Label className="text-slate-300 text-sm">อัพโหลดรูปภาพ (ถ้ามี)</Label>
                <div className="mt-1 border border-dashed border-slate-600 rounded-lg p-3 text-center">
                  ...
                  <label htmlFor="balance-img" className="cursor-pointer flex flex-col items-center gap-1 text-slate-400 hover:text-slate-300">
            ...
            <div className="flex gap-2 mt-4">
              <Button variant="outline" className="flex-1 border-slate-600 text-slate-300" onClick={() => { setStep("process"); setSelectedProcess(null); }}>ยกเลิก</Button>
              <Button className="flex-1 bg-teal-600 hover:bg-teal-700" onClick={submitProcess} disabled={loading} data-testid="btn-confirm-balance">
        ...
        {step === "done" && (
          <Card title="✅ บันทึกสำเร็จ!" subtitle={`P${selectedProcess}: ${PROCESSES[(selectedProcess || 1) - 1]?.label}`}>
            <div className="text-center py-4">
              ...
              <p className="text-slate-300 text-sm">หน่วยที่ {unitData?.unitNo} · {employeeName}</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1 border-slate-600 text-slate-300 hover:bg-slate-700" onClick={resetAll}>พนักงานใหม่</Button>
...
function Card({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div className="bg-slate-800 rounded-2xl border border-slate-700 p-5 shadow-xl">
      <h2 className="text-white font-bold text-lg mb-0.5">{title}</h2>
...
function UnitProgress({ current }: { current: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {PROCESSES.map(p => (
        <div key={p.no} className={`w-3 h-3 rounded-full ${current >= p.no ? "bg-emerald-500" : "bg-slate-600"}`} title={p.short} />
      ))}
    </div>
  );
}
